...
class JobSearchEngine:
    ...
    def search_jobs(self, query: str, resume_skills: List[str]) -> str:
        """
    Search real jobs and rank them against candidate skills
    """

        try:
            enhanced_query = f"{query} jobs hiring now requirements salary remote"

            search_results = self.tavily.run(enhanced_query)
...
Task:
Extract the TOP 5 most relevant job opportunities.
...
3. Location
4. Required Skills
5. Match Percentage (based on candidate skills)
6. Missing Skills
7. Short Recommendation

Format nicely in markdown.

Only return real jobs found in the search results.
"""
...
class HiringCopilotAgent:
    ...
    def __init__(self, gemini_api_key: str):
        ...
        self.job_search = JobSearchEngine(self.llm)
...
"""
    ...
    def _search_jobs_tool(self, query: str) -> str:
        ...
        return self.job_search.search_jobs(
        query,
        resume_skills
    )
